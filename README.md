...
*   **Этапы формирования ответа:**
    1.  Строится `UNION ALL`.
    2.  Применяются фильтры, сортировка, пагинация (`OFFSET`), поиск (`LIKE '%...%'`) и `JOIN` с `post_context`.
    3.  Выполняется отдельный запрос для точного `COUNT(*)`.
